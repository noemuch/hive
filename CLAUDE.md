...
- **CI-bypass incident guard (post-merge)**: if the reviewer ever merges a PR while required checks failed (e.g. prompt-level guard bypassed, admin PAT race), a post-step detects it and opens a `priority:critical` incident issue. Reactive — doesn't prevent, but surfaces immediately. Dedups on PR-number title prefix so two reviewer runs on the same merge don't open two issues.
...
- **Branch protection `enforce_admins: true`**: the `main` branch has `enforce_admins: true`, so even an admin-scoped PAT (`NOEMUCH_PAT`) cannot merge while the required `Tests` check is failing or pending. GitHub itself rejects the merge — this is the load-bearing CI gate. Promoted from a prompt-level guarantee on 2026-04-26 after four CI-bypass-incident races in 24h (#322, #323, #324, #326). Restore with `gh api -X PUT repos/noemuch/hive/branches/main/protection -F enforce_admins=false ...` if it ever needs to be temporarily disabled (e.g. emergency hot-fix to main).
- **Reviewer CI wait (defense-in-depth)**: the reviewer prompt still runs `gh pr checks --required --watch --fail-fast` before `gh pr merge`. With `enforce_admins: true` (above) the merge endpoint already blocks bad merges at the GitHub level; this prompt step is kept as a redundant client-side wait so the reviewer fails fast with a clear log line instead of an opaque GitHub 405. If the LLM ever skipped the line, the post-merge incident guard would flag it.
