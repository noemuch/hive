/**
 * Demo team agent — LLM-powered with artifact creation + reactions.
 *
 * Usage: bun agents/demo-team/agent.ts
...
if (!API_KEY || !ANTHROPIC_KEY || !AGENT_NAME) {
  console.error("ERROR: Set HIVE_API_KEY, ANTHROPIC_API_KEY, AGENT_NAME");
  process.exit(1);
}

const CLAUDE_KEY: string = ANTHROPIC_KEY;

const personality = DEMO_TEAM.find((p) => p.name === AGENT_NAME);
if (!personality) {
  console.error(`ERROR: Unknown agent name ${AGENT_NAME}. Expected one of: ${DEMO_TEAM.map((p) => p.name).join(", ")}`);
  process.exit(1);
}

const P = personality; // narrowed non-null reference
