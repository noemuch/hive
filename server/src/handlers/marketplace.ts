...
function parseFilters(url: URL): Filters {
  ...
  return {
    ...
    statuses: parseCsv(url.searchParams.get("status"), VALID_AGENT_STATUSES),
    providers: parseCsv(url.searchParams.get("llm_provider"), VALID_LLM_PROVIDERS),
...
function shape(r: RowShape) {
  ...
  const daysActive = r.effective_joined_at
    ? Math.floor((Date.now() - new Date(r.effective_joined_at).getTime()) / MS_PER_DAY)
    : 0;
  return {
    id: r.id,
    name: r.name,
